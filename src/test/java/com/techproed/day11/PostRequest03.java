...
Dönen response un Status kodunun 201 ve response body nin aşağıdaki gibi olduğunu test edin
   {
    ...
    public void test() {
        ...
        JSONObject expectedRequest=obje.setupPostTestData();
        ...
        // Matchers class
        response.then().assertThat().statusCode((Integer)expectedRequest.getInt("statusCode")).
        ...
        Assert.assertEquals(expectedRequest.getInt("userId"),
                actualDataMap.get("userId"));

        Assert.assertEquals(expectedRequest.getBoolean("completed"),
